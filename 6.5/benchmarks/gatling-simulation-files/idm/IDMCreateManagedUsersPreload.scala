...
class IDMCreateManagedUsersPreload extends Simulation {
    ...
    def getGeneratedUser(userId: String) : String = {
        val stringJson: String =
            """  {"userName": "preloadtestuser%s",
                  "givenName": "preloadgivenname%s",
                  "sn": "preloadtester%s",
                  "mail": "preloadtestuser%s@forgerock.com",
            ...
            feed(userFeeder)
                .exec(
                    http("Create managed user via PUT")
                        .put(idmUrl + "/openidm/managed/user/preloadtestuser${id}")
