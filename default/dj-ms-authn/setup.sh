...
# Base DN is usually set by the helm chart, but in this example we will directly set it
...
if [ -d "${DIR}"/ldif ]; then
   echo "Found optional schema files in ./ldif. Will load them"
  for file in "${DIR}"/ldif/*;  do
      echo "Loading $file"
